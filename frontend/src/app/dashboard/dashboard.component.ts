...
export class DashboardComponent {
  private readonly dashboardService = inject(DashboardService); private readonly changeDetector = inject(ChangeDetectorRef); summary: DashboardSummary | null = null; loading = true; error = '';
  constructor() { this.load(); }
  load(): void { this.loading = true; this.error = ''; this.changeDetector.markForCheck(); this.dashboardService.getSummary().pipe(finalize(() => { this.loading = false; this.changeDetector.markForCheck(); })).subscribe({ next: (summary) => { this.summary = summary; this.changeDetector.markForCheck(); }, error: () => { this.error = 'Dashboard data could not be loaded. Check that the API is running and try again.'; this.changeDetector.markForCheck(); } }); }
